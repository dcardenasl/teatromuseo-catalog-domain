...
final class CacheInvalidationOutbox
{
    ...
    public function append(array $scopes, string $source = 'catalog_automatic'): int
    {
        ...
        try {
            ...
            throw new \RuntimeException('Could not encode catalog cache invalidation payload.', 0, $exception);
        }
        ...
        $db->table('cache_invalidation_outbox')->insert([
            ...
            'payload' => $payload,
            'source' => mb_substr(trim($source) !== '' ? trim($source) : 'catalog_automatic', 0, 64),
    ...
    public function claim(int $limit = 20, int $leaseSeconds = 300): array
    {
        ...
        $query = $db->table('cache_invalidation_outbox')
            ->where('dispatched_at', null)->where('available_at <=', $now)
        ...
        foreach ($rows as $row) {
            ...
            if (! is_array($payload) || ! is_array($payload['scopes'] ?? null)) {
                $this->release($id, $token, 'Invalid catalog cache outbox payload.', 3600);
                continue;
            }
            $claimed[] = ['id' => $id, 'payload' => $payload, 'source' => (string) ($row['source'] ?? 'catalog_automatic'), 'attempts' => (int) ($row['attempts'] ?? 0), 'lock_token' => $token];
        }
